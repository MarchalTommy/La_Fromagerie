...
| A valid address is rejected — "ne semble pas sur un parcours de livraison" — for a city that IS on a path | Is the in-memory path list simply **empty**? Everything else (city normalization, geocoding) is downstream of that and is usually innocent | **Poisoned path cache** (fixed 2026-07-29, see archaeology §14): an offline first launch made Firestore's `get()` succeed with zero documents, which used to be persisted as truth — the refresh flag was cleared and the Room cache wiped, permanently. If the symptom reappears on a build that HAS the fix, suspect instead: `cities`/`postcodes` arrays of different length in the Firestore doc (`zip` truncates silently, dropping trailing cities), or a path dropped whole because one of its cities failed to reverse-geocode. | Test the exact geocoding call the app makes, for each city of the suspect path: `curl -s "https://api-adresse.data.gouv.fr/search/?q=<City>-<zip>&type=municipality"` — 0 features means that one city kills the whole path. Then compare `cities` and `postcodes` lengths in the `delivery_paths` doc (Firebase console, read-only). Note the client flavor has **no manual path selector**, so you cannot use it as a discriminator. | `fromagerie-failure-archaeology` (§14), `fromagerie-delivery-logistics-reference` |
