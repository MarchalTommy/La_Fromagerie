...
| I3 | **Firestore is the source of truth; Room is a cache invalidated by the `database_update` timestamp.** `GetLastFirestoreDatabaseUpdateUseCase` compares the remote timestamp to the local cache. `FromagerieDatabase` (entities `ProductEntity`, `PathEntity`) registers real migrations **and** keeps `fallbackToDestructiveMigration(true)` behind them as a net for unhandled jumps. | Offline reads + cheap reloads without hammering Firestore. | Stale product/price/path data shown to customers if the invalidation timestamp is not bumped on writes. **Write a real additive migration rather than leaning on the destructive fallback** — a wiped cache plus a first launch with no network makes every address undeliverable, a bug that actually shipped. Schema version, columns and migration list live in **fromagerie-firestore-data-model** §5 (one home per fact); field-level Firestore contract is in the same skill. |
