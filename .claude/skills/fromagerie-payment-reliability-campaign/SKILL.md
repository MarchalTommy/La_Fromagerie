...
**Candidate: webhook/server-side confirmation via a Cloud Function. — OWNER-DESIGNATED next reliability improvement for the hosted path (2026-07-10).** A function receiving SumUp status callbacks could write the order status server-side, removing the dependency on the customer's device entirely — the strongest possible fix for F7/F8, and for the hosted path's residual app-launch dependency. **Prerequisite CLEARED 2026-07-07:** the backend entered version control at commit `ecda756` (`la-fromagerie-backend/` tracked; `functions/.env` gitignored — verified).

**Webhook capability VERIFIED 2026-07-10** against SumUp docs (`developer.sumup.com/online-payments/webhooks/` + `.../hosted-checkout`):
- SumUp supports exactly one event: `CHECKOUT_STATUS_CHANGED`. You subscribe by passing a `return_url` when creating the checkout — **separate from the button's `redirect_url`; the two coexist**, so adding the webhook does NOT touch the existing return button.
- Payload is minimal and NOT trustworthy on its own: `{ "event_type": "CHECKOUT_STATUS_CHANGED", "id": "<checkout-id>" }`. The handler MUST re-fetch `GET v0.1/checkouts/{id}` to confirm the real status (SumUp's docs: "always verify … by calling a relevant SumUp's API").
- Failed deliveries retry at 1 min / 5 min / 20 min / 2 h.
- SumUp's own guidance: "use the checkout status and webhook events as the source of truth for your backend order state."

**Design sketch (build on its own branch; deploy needs Tommy — backend + prod):** add an HTTPS function `sumupWebhook` next to `createSumUpCheckout` in `la-fromagerie-backend/functions/src/index.ts`; on `CHECKOUT_STATUS_CHANGED` it GETs the checkout, and on `PAID` whose `amount` matches the `orders/{checkout_reference}.total_price` (cents) it writes the order status server-side (idempotent, `update()` only). Set `return_url` alongside the existing `redirect_url` in the checkout-creation body. This makes finalization device-independent — the FPW safety net and the app-launch dependency (F8) become backups, not the primary path. Still to confirm before building: webhook signature/verification (is the endpoint authenticated, or must we treat every hit as unauthenticated and rely on the API re-fetch?), and idempotency against the retry schedule above.
...
1. **Durable safety net — ✅ CLOSED 2026-07-08:** `CheckoutViewModel.getSumUpPaymentLink` schedules the FPW marker BEFORE opening the custom tab. Because the Cloud Function only returns the payment URL (no session id), the marker carries `checkoutId = null` + `expectedAmountCents` (`PendingPaymentFinalization` gained both fields; DataStore keys extended, old markers still parse). `FinalizePaymentWorker` reconciles id-less markers through `SumUpDataSource.pollHostedCheckoutStatus(reference = orderId, expectedAmountCents)`, which only accepts a PAID session whose amount matches the order total (a mismatched PAID is logged and ignored — fail-safe to PENDING for human reconciliation) and maps all-FAILED to CANCELED. Unit-tested (`SumUpDataSourceTest`, `PaymentFinalizationUseCasesTest`, `CheckoutViewModelTest` hosted cases). Same guarantee level as the Google Pay path now: worker give-up still depends on a later app launch (F8) — webhook confirmation (above) remains the strong fix.
2. **Return-path verification — ✅ HARDENED (#49 resilient poll, #50 reliable trigger, 2026-07-10):** `verifySumUpWebCheckoutStatus()` now runs a bounded (~30 s) resilient poll (not a single lookup) with three honest outcomes — PAID → success; FAILED → cart kept; unknown → "Nous vérifions votre paiement…", never a false "not charged". And the trigger no longer depends on the flaky custom-scheme deep link: `CheckoutViewModel.onReturnedFromHostedCheckout()` gates a single verification fired from EITHER the deep-link callback OR a `LifecycleEventEffect(ON_RESUME)` in `CheckoutScreen` (flag armed in `getSumUpPaymentLink`). Since SumUp's hosted page never auto-redirects (VERIFIED 2026-07-10 — only ever a manual, HTTPS-only button), ON_RESUME is what actually brings the success screen up. UX caveat (owner-accepted): a plain cancel also triggers the ~30 s verify → the "nous vérifions" message (safe, mildly misleading for a cancel).
...
7. **App-killed outcome surfacing — ✅ SHIPPED (#50, 2026-07-10):** `FinalizePaymentWorker.finalize()` records a `PaymentOutcome(orderId, clientName, isPaid)` in the checkout DataStore (3 keys) when it reconciles a payment while the app is away; the client `NavGraph` home destination consumes it once at next launch (`ConsumePaymentOutcomeUseCase` = read-and-clear) → routes to `AfterPaymentScreen` on PAID, snackbar ("panier conservé") on FAILED. The in-app flow clears the pending marker first in the foreground case, so the worker no-ops and never double-notifies. Applies to BOTH payment paths (the worker is shared). **Redirect decision (owner 2026-07-10): leave the `lafromagerie://checkout-callback` custom scheme as-is** — the ON_RESUME fallback (item 2) removed the dependency on it firing, so an HTTPS App Link (verified domain + `assetlinks.json` + backend redirect change) is NOT worth it; the next step up is the webhook (Phase 5), not the redirect.
...
| 9 | Hosted-checkout path via `createSumUpCheckout` | **LANDED as working WIP** (`ecda756`) | remaining before "done": ~~FPW safety net~~ (2026-07-08), ~~single-shot verification~~ (resilient poll #49 + ON_RESUME trigger #50, 2026-07-10), ~~app-killed outcome surfacing~~ (#50), two-path duplicate analysis, hardcoded URL — see the LANDED block above |
